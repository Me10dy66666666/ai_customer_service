...
import * as customerServiceGateway from '@deepseek-ai/dsh-customer-service-gateway'
...
export interface Config {
  ...
  /** Optional trusted Backend HTTP/SSE BFF; disabled for the stdio-only default. */
  customerServiceGateway?: customerServiceGateway.Config | false
}
...
export const Config: z<Config> = z.object({
  ...
  customerServiceGateway: z.union([z.const(false), customerServiceGateway.Config]).default(false),
})
/* jscpd:ignore-end */
...
export async function apply(ctx: Context, config: Config): Promise<void> {
  ...
  await ctx.effect(async function* () {
    ...
    if (config.customerServiceGateway !== false && config.customerServiceGateway !== undefined) {
      const gateway = ctx.plugin(customerServiceGateway, config.customerServiceGateway)
      await gateway
      yield gateway.dispose
    }
