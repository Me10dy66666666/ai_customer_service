...
> 文档状态：基线审计记录。当前落地状态和验收结果以仓库根目录 `README.md` 与 `ENGINEERING_AUDIT.md` 为准；本文保留的 ChromaDB/旧工具描述用于说明改造前风险。
...
| P1 | Dify/DSH Gateway 客户端缺少统一超时、重试、熔断与隔离 | 可靠性策略按读写请求拆分；写请求不做无幂等重试 |
