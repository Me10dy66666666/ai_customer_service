...
export class DocumentChunkingService {
  ...
  private readonly pdrParentChunkSize: number;
  private readonly pdrParentOverlap: number;
  private readonly pdrChildChunkSize: number;
  private readonly pdrChildOverlap: number;

  private readonly SEPARATORS = ["\n\n", "\n", "。", "！", "？", "；", "，", " "];

  public constructor(options: ChunkingOptions = {}) {
    this.chunkSize = options.chunkSize ?? 800;
    this.chunkOverlap = options.chunkOverlap ?? 150;
    this.pdrParentChunkSize = options.pdrParentChunkSize ?? 2000;
    this.pdrParentOverlap = options.pdrParentOverlap ?? 200;
    this.pdrChildChunkSize = options.pdrChildChunkSize ?? 400;
    this.pdrChildOverlap = options.pdrChildOverlap ?? 50;
  }
  ...
  public splitWithParent(text: string): { parentChunks: Chunk[]; childChunks: Chunk[] } {
    const parentChunks = this.splitRecursive(text, {
      chunkSize: this.pdrParentChunkSize,
      chunkOverlap: this.pdrParentOverlap
    });
    const childChunks = this.splitRecursive(text, {
      chunkSize: this.pdrChildChunkSize,
      chunkOverlap: this.pdrChildOverlap
    });
    ...
    for (let i = 0; i < childChunks.length; i++) {
      ...
      const parentIdx = parentChunks.findIndex((p: Chunk | undefined) => {
        if (!p) return false;
